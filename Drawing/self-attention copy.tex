...
\node[emb, right of=e3, node distance=2cm] (e4) {$\bm{v}_{t}$};

\node[below of=e1, node distance=1.5cm] (i1) {$v_1$};
\node[below of=e2, node distance=1.5cm] (i2) {$\cdots$};
\node[below of=e4, node distance=1.5cm] (i4) {$v_t$};
...
\node[proj, above of=tt1, node distance=0.95cm] (proj4) {\scriptsize Projection};
\node[proj, above of=tt3, node distance=0.95cm] (proj3) {\scriptsize Projection};
...
\node[above of=proj1, node distance=0.8cm] (o4) {$v_{t+1}$};
\node[above of=proj4, node distance=0.8cm] (o31) {$v_{2}$};
\node[above of=proj3, node distance=0.8cm] (o11) {$v_{t}$};
\node[above of=tt4, node distance=0.55cm, xshift=0.25cm] (oa4) {\scriptsize$\bm{h}_{t}^L$};

\draw[FARROW] (tt4) -> (proj1) ;
\draw[FARROW] (proj1) -> (o4) ;
\draw[FARROW] (proj4) -> (o31) ;
\draw[FARROW] (proj3) -> (o11) ;
\draw[FARROW] (tt1) -> (o1) ;
\draw[FARROW] (tt3) -> (o3) ;
...
\draw[FARROW] (i4.east) -- ++(0.45, 0) |-  (e4.east) ;
...
\node[above of=st\x, node distance=1cm] (sd\x) {$\vdots$};
\node[transformer, above of=sd\x, node distance=0.8cm, minimum height=1.2em, inner sep=0, outer sep=0] (stt\x) {Trm};
}
\node[below of=sa1, node distance=0.7cm] (si1) {$v_1$};
\node[below of=sa3, node distance=0.7cm] (si3) {$v_{t-1}$};
\node[below of=sa4, node distance=0.7cm] (si4) {\color{flamingo}$v_{t}$};
...
\node[above of=stt3, node distance=0.7cm] (so3) {\color{flamingo}$v_{t}$};
...
{
    \draw[FARROW] (sa\x.north) -> (st\y.south) ;
    \draw[FARROW] (st\x.north) -> (sd\y.south) ;
\draw[FARROW]  ($(sd\x.north)+(0, -0.15)$) -> (stt\y.south) ;
}
}
...
\foreach \x in {1,3,4}
{
\node[emb, below of=utt\x, node distance=8cm, xshift=-8cm, minimum height=1.2em] (qsa\x) {};
\node[transformer, above of=qsa\x, node distance=0.8cm, minimum height=1.2em, inner sep=0, outer sep=0] (qst\x) {Trm};
\node[above of=qst\x, node distance=1cm] (qsd\x) {$\vdots$};
\node[transformer, above of=qsd\x, node distance=0.8cm, minimum height=1.2em, inner sep=0, outer sep=0] (qstt\x) {Trm};
}

\node[below of=qsa1, node distance=0.7cm] (qsi1) {$v_1$};
\node[below of=qsa3, node distance=0.7cm] (qsi3) {$v_{t-1}$};
\node[below of=qsa4, node distance=0.7cm] (qsi4) {$v_{t}$};
\node[right of=qsa1, node distance=1.2cm, minimum height=1.2em] (qsa2) {$\ldots$};

\node[below of=qsa2, node distance=0.7cm, minimum height=1.2em] (qsi2) {$\ldots$};
\node[above of=qsa2, node distance=0.8cm, minimum height=1.2em] (qst2) {$\ldots$};
\node[above of=qst2, node distance=0.8cm, minimum height=1.2em] (qstt2) {$\ldots$};
%\node[above of=qstt1, node distance=0.7cm] (qso1) {$v_2$};
%\node[above of=qstt3, node distance=0.7cm] (qso3) {$v_{t}$};
\node[above of=qstt4, node distance=0.7cm] (qso4) {$v_{t+1}$};

\foreach \x in {1,3,4}
{
\draw[FARROW] ($(qsi\x.north)-(0,0.08)$) -> (qsa\x) ;
%\draw[FARROW] (sa\x.north) -> (st\x.south) ;
%\draw[FARROW] (st\x.north) -> (stt\x.south) ;

}
\draw[FARROW] (qstt4) -> ($(qso4.south)-(0,-0.08)$) ;
...
\foreach \y in {1,3,4}
{
    \draw[FARROW] (qsa\x.north) -> (qst\y.south) ;
    \draw[FARROW] (qst\x.north) -> (qsd\y.south) ;
\draw[FARROW]  ($(qsd\x.north)+(0, -0.15)$) -> (qstt\y.south) ;
}
}
...
%%\draw[FARROW] (sa\x.north) -> (st\x.south) ;
%%\draw[FARROW] (st\x.north) -> (stt\x.south) ;
%\draw[FARROW] (qstt\x) -> ($(qso\x.south)-(0,-0.08)$) ;
%}
%
%%\foreach \x/\y in {1/3, 1/4, 3/4}
%%{
